define([ 'jquery', 'underscore', 'backbone',
		...
		"pages/RestaurantPage/models/bucket_model" ], function($, _, Backbone,
		Bucket, Orders, Tickets, BucketTemplate, bucketModel) {
	...
	var bucketView = Backbone.View.extend({
		...
		cartDeleteTicket : function(event) {
			...
			$(this.el).find("#cartTicketToDelete_" + id).fadeOut(1000,
					function() {
		...
		drag : function() {
			...
			div.mousedown(function(e) {
				...
				/*
				 * this.onmouseout = function() { document.onmousemove =
		...
		render : function(option) {
			try {
				...
				if (parseInt(document.cookie.match(new RegExp("role" + '=([^;]+)'))[1]) == 2 || parseInt(document.cookie.match(new RegExp("role" + '=([^;]+)'))[1]) == 3) {
					...
					if (bucket.length > 0) {
						self.$el.find($("#bucket")).css("display", "block");
						self.$el.find($(".cartAddOrder button")).prop(
								"disabled", false);
					}
					cartView.drag();
				}
